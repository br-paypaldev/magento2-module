<?php
namespace PayPalBR\PayPal\Model\PayPalBCDC;
 ...
 *
 * This class provides access to Magento 2 configuration over PayPal BCDC
 *
...
class ConfigProvider
{
     ...
     */
    const XML_PATH_MERCHANT_ID = 'payment/paypalbr/merchant_id';
     ...
     * Contains the secret ID of PayPal Plus (Production)
     */
    const XML_PATH_WEBHOOK_ID = 'payment/paypalbr/webhook_id';
    ...
    {
        $this->config->saveConfig(self::XML_PATH_MERCHANT_ID, $merchantId, 'default', 0);
    }

    public function getMerchantId()
    {
        return $this->scopeConfig->getValue(self::XML_PATH_MERCHANT_ID, ScopeInterface::SCOPE_STORE);
    }

    public function setClientId($clientId)
    {
        if ($this->isModeProduction()) {
            $this->config->saveConfig(self::XML_PATH_CLIENT_ID_PROD, $clientId, 'default', 0);
        } else {
            $this->config->saveConfig(self::XML_PATH_CLIENT_ID_SBOX, $clientId, 'default', 0);
        }
    }
    ...
    {
        if ($this->isModeProduction()) {
            $this->config->saveConfig(self::XML_PATH_SECRET_ID_PROD, $secretId, 'default', 0);
        } else {
            $this->config->saveConfig(self::XML_PATH_SECRET_ID_SBOX, $secretId, 'default', 0);
        }
    }
}
